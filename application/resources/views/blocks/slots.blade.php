...
<div class="row custom-slot-list">
    @for($a=0; $a<$hours;$a++)
        @if($list_slot[$a]['is_available'])
            <div class="col-md-3">
                <a class="btn btn-outline-{{ \Session::get('booking_type_id') == 2 ? 'yellow' : 'dark' }} btn-lg btn-block btn-slot" data-slot-time="{{ $list_slot[$a]['slot'] }}">{{ $list_slot[$a]['slot'] }}</a>
            </div>
        ...
        @else
            <div class="col-md-3">  <!-- btn-outline-dark  LA -->
